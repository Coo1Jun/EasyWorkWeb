<template>
  ...
  >
    ...
    >
      ...
      <SelectColumn />
      <!-- 文件展示模式 -->
      <div v-if="screenWidth <= 768" class="change-file-model">
        ...
        >
          <el-radio-button :label="0">
            <i class="iconfont icon-liebiao1" /> 列表
          </el-radio-button>
    ...
    <!-- 多选文件下载，页面隐藏 -->
    <a ref="batchDownloadRef" target="_blank" :href="batchDownloadLink" />
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    // 批量下载文件链接
    batchDownloadLink() {
      return `/fileTransfer/batchDownloadFile?userFileIds=${this.selectedFiles
