<template>
  <transition name="el-fade-in-linear el-fade-in">
    <div v-show="visible" class="img-preview-wrapper">
      <!-- 顶部信息栏 & 工具栏 -->
      <div v-if="visible" class="tip-wrapper">
        <i
          class="fold-icon"
          :class="
            isShowMinImgList
        ...
        <div class="tool-wrapper">
          ...
          <el-tooltip effect="dark" placement="bottom">
            <div slot="content" style="line-height: 1.5">
              ...
              4. 鼠标滚轮可放大、缩小图片<br>
              5. 点击左上角
              <i class="el-icon-d-arrow-left" /> 图标可折叠、展开缩略图<br>
            </div>
      ...
      <!-- 左侧缩略图 -->
      <ul v-show="isShowMinImgList" ref="minImgListRef" class="min-img-list">
        <li
...
export default {
  ...
  watch: {
    ...
    visible(val) {
      if (val) {
        ...
        this.isShowMinImgList =
					localStorage.getItem('file_img_preview_show_min') !== 'false'
        this.$nextTick(() => {
    ...
    isShowMinImgList(val) {
      localStorage.setItem('file_img_preview_show_min', val)
    }
  },
  methods: {
    /**
