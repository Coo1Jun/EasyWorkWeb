<template>
  ...
  >
    <div v-if="unzipMode === 2" class="unzip-tree-wrapper">
      <!-- 选择的目标路径 -->
    ...
    <div v-else class="unzip-text">
      <i class="el-icon-loading" /> 正在解压缩，请稍等片刻...
    </div>
    <div v-if="unzipMode === 2" slot="footer" class="dialog-footer">
...
export default {
  ...
  methods: {
    ...
    /**
		 ...
		 */
    handleUnzipFile() {
      // unzipMode解压模式:0-解压到当前文件夹，1-自动创建该文件名目录，并解压到目录里，2-手动选择解压目录
      let reqData = {
      ...
      // fileApi.unzipFile(reqData).then((res) => {
      //   console.log(res)
